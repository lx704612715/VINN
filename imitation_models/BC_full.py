...
class BC_Full:
    def __init__(self, params):
        ...
        if self.params['wandb'] == 1:
            ...
            if self.params['dataset'] == 'CustomDataset':
        ...
        if self.params['dataset'] == 'PushDataset' or self.params['dataset'] == 'StackDataset':
            ...
            self.dataLoader_val = DataLoader(self.img_data_val, batch_size=self.params['batch_size'], shuffle=True,
                                             pin_memory=True)
            self.dataLoader_test = DataLoader(self.img_data_test, batch_size=self.params['batch_size'], shuffle=True,
                                              pin_memory=True)
    ...
    def train(self):
        ...
        for epoch in range(self.params['epochs']):
            ...
            if self.params['dataset'] == 'PushDataset' or self.params['dataset'] == 'StackDataset':
                ...
                self.translation_loss_val = 0
                self.translation_loss_test = 0
            if self.params['dataset'] == 'HandleData' or self.params["dataset"] == "CustomDataset":
                self.translation_loss_train = 0
                self.rotation_loss_train = 0
                self.gripper_loss_train = 0
                self.translation_loss_val = 0
    ...
    def get_val_losses(self, fraction, times):
        ...
        for _ in range(times):
            ...
            self.dataLoader_train = DataLoader(self.img_data_train, batch_size=self.params['batch_size'], shuffle=True,
                                               pin_memory=True)
    ...
    def get_test_losses(self, fraction, times):
        ...
        for _ in range(times):
            ...
            self.optimizer = torch.optim.Adam(
                list(self.resnet.parameters()) + list(self.translation_model.parameters()), lr=self.params['lr'])
    ...
    def save_model(self, epoch):
        ...
        if self.params['dataset'] == 'PushDataset':
            torch.save({'model_state_dict': self.translation_model.state_dict()
                        }, self.params['save_dir'] + 'PushModel_translation_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')
            torch.save({'model_state_dict': self.resnet.state_dict()
                        }, self.params['save_dir'] + 'push_resnet_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')

        if self.params['dataset'] == 'StackDataset':
            torch.save({'model_state_dict': self.translation_model.state_dict()
                        }, self.params['save_dir'] + 'StackModel_translation_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')
            torch.save({'model_state_dict': self.resnet.state_dict()
                        }, self.params['save_dir'] + 'stack_resnet_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')

        if self.params['dataset'] == 'HandleData':
            torch.save({'model_state_dict': self.translation_model.state_dict()
                        }, self.params['save_dir'] + 'HandleModel_translation_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')
            torch.save({'model_state_dict': self.rotation_model.state_dict()
                        }, self.params['save_dir'] + 'HandleModel_rotation_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')
            torch.save({'model_state_dict': self.gripper_model.state_dict()
                        }, self.params['save_dir'] + 'HandleModel_gripper_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')

            torch.save({'model_state_dict': self.resnet.state_dict()
                        }, self.params['save_dir'] + 'HandleModel_resnet_full_pretrained' + str(
                self.params['pretrain_encoder']) + '_' + str(epoch) + '.pt')
